...
var Platform = require('../../../../src/Platform/index'),
    ...
    store;
$testGroup('SQLiteStore - read tests').beforeEachAsync(function() {
...
}), $test('Read entire table').checkAsync(function() {
  var rows = [{
    id: 1,
    int: 101,
    str: 'text1'
...
}), $test('Read - query on a bool property').checkAsync(function() {
  var record1 = {
    id: 1,
    flag: false
  };
...
}), $test('Read - query on a string property').checkAsync(function() {
  var record1 = {
    id: '1',
    name: 'a'
  };
...
}), $test('Read - query on a date property').checkAsync(function() {
  var record1 = {
    id: '1',
    time: new Date(2011, 10, 11, 12, 13, 14)
  };
  var record2 = {
    id: '2',
    time: null
...
}), $test('Read - query on an integer property').checkAsync(function() {
  var record1 = {
    id: '1',
    val: 1
  };
...
}), $test('Read - query on a float property').checkAsync(function() {
  var record1 = {
    id: '1',
    val: 1.1
  };
...
}), $test('simple select').checkAsync(function() {
  var rows = [{
    id: 1,
    int: 101,
    str: 'text1'
...
}), $test('select invalid columns').checkAsync(function() {
  var row = {
    id: 1,
    int: 101,
    str: 'text1'
...
}), $test('select same columns more than once').checkAsync(function() {
  var row1 = {
    id: 1,
    int: 101,
    str: 'text1'
...
}), $test('query a non-existent table').checkAsync(function() {
  return store.read(new Query('nonexistenttable')).then(function(results) {
    $assert.fail('failure expected');
  }, function(error) {});
}), $test('query an invalid table name').checkAsync(function() {
  return store.read(new Query('*')).then(function(results) {
    $assert.fail('failure expected');
  }, function(error) {});
}), $test('read performed without any arguments').checkAsync(function() {
  return store.defineTable({
    name: storeTestHelper.testTableName,
    columnDefinitions: {
...
}), $test('read invoked with extra parameters').description('Check that promise returned by read is either resolved or rejected even when invoked with extra parameters').checkAsync(function() {
  return store.defineTable({
    name: storeTestHelper.testTableName,
    columnDefinitions: {
      id: MobileServiceSqliteStore.ColumnType.String,
