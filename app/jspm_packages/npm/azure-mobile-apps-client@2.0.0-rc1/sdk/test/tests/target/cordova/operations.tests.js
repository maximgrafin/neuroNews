...
$testGroup('operations tests').beforeEachAsync(function() {
...
}).tests($test('verify initialization').checkAsync(function() {
...
}), $test('basic logging').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store);
  return operationTableManager.initialize().then(function() {
...
}), $test('operation ID generation').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store),
      item1 = {id: 'abc'},
      item2 = {id: 'def'};
  return operationTableManager.initialize().then(function() {
    return operationTableManager.getLoggingOperation(storeTestHelper.testTableName, 'insert', item1);
  }).then(function(op) {
    return store.executeBatch([op]);
  }).then(function() {
    ...
    return operationTableManager.getLoggingOperation(storeTestHelper.testTableName, 'delete', item1);
  }).then(function(op) {
    return store.executeBatch([op]);
  }).then(function() {
    return operationTableManager.getLoggingOperation(storeTestHelper.testTableName, 'insert', item1);
    ...
    return operationTableManager.getLoggingOperation(storeTestHelper.testTableName, 'insert', item2);
  }).then(function(op) {
    return store.executeBatch([op]);
  }).then(function() {
...
}), $test('getLoggingOperation insert').checkAsync(function() {
  return performActionsAndVerifySuccess(['insert'], [{
    id: 1,
    action: 'insert'
  }]);
}), $test('getLoggingOperation insert, update').checkAsync(function() {
  return performActionsAndVerifySuccess(['insert', 'update'], [{
    id: 1,
    action: 'insert'
  }]);
}), $test('getLoggingOperation insert, delete').checkAsync(function() {
  return performActionsAndVerifySuccess(['insert', 'delete'], []);
}), $test('getLoggingOperation insert, lock, update').checkAsync(function() {
...
}), $test('getLoggingOperation insert, lock, delete').checkAsync(function() {
...
}), $test('getLoggingOperation update').checkAsync(function() {
  return performActionsAndVerifySuccess(['update'], [{
    id: 1,
    action: 'update'
  }]);
}), $test('getLoggingOperation update, update').checkAsync(function() {
  return performActionsAndVerifySuccess(['update', 'update'], [{
    id: 1,
    action: 'update'
  }]);
}), $test('getLoggingOperation update, delete').checkAsync(function() {
  return performActionsAndVerifySuccess(['update', 'delete'], [{
    id: 1,
    action: 'delete'
  }]);
}), $test('getLoggingOperation update, lock, update').checkAsync(function() {
...
}), $test('getLoggingOperation update, lock, delete').checkAsync(function() {
...
}), $test('getLoggingOperation delete').checkAsync(function() {
  return performActionsAndVerifySuccess(['delete'], [{
    id: 1,
    action: 'delete'
  }]);
}), $test('getLoggingOperation delete, delete').checkAsync(function() {
  return performActionsAndVerifySuccess(['delete', 'delete'], [{
    id: 1,
    action: 'delete'
  }]);
}), $test('getLoggingOperation delete, lock, delete').checkAsync(function() {
...
}), $test('getLoggingOperation insert, insert').checkAsync(function() {
  return performActionsAndVerifyError(['insert'], 'insert');
}), $test('getLoggingOperation insert, lock, insert').checkAsync(function() {
  return performActionsAndVerifyError(['insert', 'lock'], 'insert');
}), $test('getLoggingOperation update, insert').checkAsync(function() {
  return performActionsAndVerifyError(['update'], 'insert');
}), $test('getLoggingOperation update, lock, insert').checkAsync(function() {
  return performActionsAndVerifyError(['update', 'lock'], 'insert');
}), $test('getLoggingOperation delete, insert').checkAsync(function() {
  return performActionsAndVerifyError(['delete'], 'insert');
}), $test('getLoggingOperation delete, update').checkAsync(function() {
  return performActionsAndVerifyError(['delete'], 'update');
}), $test('getLoggingOperation delete, lock, insert').checkAsync(function() {
  return performActionsAndVerifyError(['delete', 'lock'], 'insert');
}), $test('getLoggingOperation delete, lock, update').checkAsync(function() {
  return performActionsAndVerifyError(['delete', 'lock'], 'update');
}), $test('readFirstPendingOperationWithData reads - insert log operation').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store);
  var logRecord1 = {
    id: 1001,
    action: 'update',
...
}), $test('readFirstPendingOperationWithData - update log operation').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store);
  var logRecord1 = {
    id: 1001,
    action: 'insert',
...
}), $test('readFirstPendingOperationWithData - delete log operation').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store);
  var logRecord1 = {
    id: 1001,
    action: 'insert',
  ...
  return operationTableManager.initialize().then(function() {
    ...
    $assert.areEqual(result, {logRecord: logRecord2});
  }, function(error) {
    $assert.fail(error);
  }).then(function() {
    return operationTableManager.readFirstPendingOperationWithData(3000);
  }).then(function(result) {
    $assert.areEqual(result, {logRecord: logRecord3});
  }, function(error) {
    $assert.fail(error);
  });
}), $test('readFirstPendingOperationWithData - first log record without data record, next log record has data record').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store);
  var logRecord1 = {
    id: 1001,
    action: 'udpate',
...
}), $test('readFirstPendingOperationWithData - first log record without data record, next log record does not exist').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store);
  var logRecord1 = {
    id: 1,
    action: 'insert',
...
}), $test('readFirstPendingOperationWithData - log record does not exist').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store);
  return operationTableManager.initialize().then(function() {
    return operationTableManager.readFirstPendingOperationWithData(-1);
  }).then(function(result) {
...
}), $test('removeLockedOperation').checkAsync(function() {
  var operationTableManager = createOperationTableManager(store);
  var logRecord1 = {
    id: 1,
    action: 'udpate',
...
}), $test('getMetadata - action is insert, new record has version').checkAsync(function() {
  return verify_getMetadata('insert', undefined, {version: testVersion}, {version: testVersion});
}), $test('getMetadata - action is insert, new record has null version').checkAsync(function() {
  return verify_getMetadata('insert', undefined, {version: null}, {version: null});
}), $test('getMetadata - action is insert, new record has no version').checkAsync(function() {
  return verify_getMetadata('insert', undefined, {}, {version: undefined});
}), $test('getMetadata - action is upsert, new record has version').checkAsync(function() {
  return verify_getMetadata('upsert', undefined, {version: testVersion}, {version: testVersion});
}), $test('getMetadata - action is upsert, new record has null version').checkAsync(function() {
  return verify_getMetadata('upsert', undefined, {version: null}, {version: null});
}), $test('getMetadata - action is upsert, new record has no version').checkAsync(function() {
  return verify_getMetadata('upsert', undefined, {}, {version: undefined});
}), $test('getMetadata - action is update, store has no such record, new record has version').checkAsync(function() {
  return verify_getMetadata('update', undefined, {version: testVersion}, {version: testVersion});
}), $test('getMetadata - action is update, store has no such record, new record has null version').checkAsync(function() {
  return verify_getMetadata('update', undefined, {version: null}, {version: null});
}), $test('getMetadata - action is update, store has no such record, new record has no version').checkAsync(function() {
  return verify_getMetadata('update', undefined, {}, {});
}), $test('getMetadata - action is update, store has record without version, new record has version').checkAsync(function() {
  return verify_getMetadata('update', {version: undefined}, {version: testVersion}, {version: testVersion});
}), $test('getMetadata - action is update, store has record without version, new record has null version').checkAsync(function() {
  return verify_getMetadata('update', {version: undefined}, {version: null}, {version: null});
}), $test('getMetadata - action is update, store has record without version, new record has no version').checkAsync(function() {
  return verify_getMetadata('update', {version: undefined}, {}, {version: null});
}), $test('getMetadata - action is update, store has record with version, new record has new version').checkAsync(function() {
  return verify_getMetadata('update', {version: 'oldversion'}, {version: testVersion}, {version: testVersion});
}), $test('getMetadata - action is update, store has record with version, new record has same version').checkAsync(function() {
  return verify_getMetadata('update', {version: testVersion}, {version: testVersion}, {version: testVersion});
}), $test('getMetadata - action is update, store has record with version, new record has null version').checkAsync(function() {
  return verify_getMetadata('update', {version: testVersion}, {version: undefined}, {version: undefined});
}), $test('getMetadata - action is update, store has record with version, new record has no version').checkAsync(function() {
  return verify_getMetadata('update', {version: testVersion}, {}, {version: testVersion});
}), $test('getMetadata - action is delete, store has no such record').checkAsync(function() {
  return verify_getMetadata('delete', undefined, {}, {});
}), $test('getMetadata - action is delete, store has record with version').checkAsync(function() {
  return verify_getMetadata('delete', {version: testVersion}, {}, {version: testVersion});
}), $test('getMetadata - action is delete, store has record with null version').checkAsync(function() {
  return verify_getMetadata('delete', {version: null}, {}, {version: null});
}), $test('getMetadata - action is delete, store has record with no version').checkAsync(function() {
  return verify_getMetadata('delete', {}, {}, {version: null});
}), $test('getMetadata - action is delete, store has record with no version, specified record has version').description('verify that version passed to getMetadata is not used in case of a delete operation').checkAsync(function() {
  return verify_getMetadata('delete', {}, {version: testVersion}, {version: null});
}), $test('getOperationForInsertingLog').description('verify that getOperationForInsertingLog uses whatever metadata is returned by getMetadata()').checkAsync(function() {
...
}), $test('getOperationForUpdatingLog').description('verify that getOperationForInsertingLog uses whatever metadata is returned by getMetadata()').checkAsync(function() {
  return verify_getOperation(function(operationTableManager) {
...
function performAction(asyncChain, operationTableManager, item, action) {
  return asyncChain.then(function() {
    if (action === 'insert' || action === 'update' || action === 'delete') {
      ...
      return operationTableManager.lockOperation(1);
