...
var Platform = require('../../../../src/Platform/index'),
    ...
    store;
$testGroup('SQLiteStore - miscellaneous tests').beforeEachAsync(function() {
  return storeTestHelper.createEmptyStore().then(function(emptyStore) {
    store = emptyStore;
  });
}).tests($test('Roundtrip non-null property values').checkAsync(function() {
  var row = {
    id: 'someid',
    object: {
      int: 1,
...
}), $test('Roundtrip null property values').checkAsync(function() {
  var row = {
    id: '1',
    object: null,
    array: null,
...
}), $test('Read table with columns missing from table definition').checkAsync(function() {
  var row = {
    id: 101,
    flag: 51,
    object: {'a': 21}
...
}), $test('ID column case insensitivity').checkAsync(function() {
  var tableDefinition = {
    name: storeTestHelper.testTableName,
    columnDefinitions: {
      Id: MobileServiceSqliteStore.ColumnType.Integer,
  ...
  return testHelper.runActions([[store, store.defineTable, tableDefinition], [store, store.upsert, storeTestHelper.testTableName, {
    ID: 101,
    somecolumn: 51
  }], [store, store.lookup, storeTestHelper.testTableName, 101], function(result) {
    $assert.areEqual(result, {
      Id: 101,
      somecolumn: 51
    });
  }, [store, store.upsert, storeTestHelper.testTableName, {
    iD: 101,
    somecolumn: 71
  }], [store, store.read, new Query(storeTestHelper.testTableName).where(function() {
    return this.iD === 101;
  })], function(result) {
    $assert.areEqual(result, [{
      Id: 101,
  ...
  }, [store, store.del, storeTestHelper.testTableName, 101], [store, store.read, new Query(storeTestHelper.testTableName)], function(result) {
    $assert.areEqual(result, []);
  }]);
}), $test('Non-ID column case insensitivity').checkAsync(function() {
  var tableDefinition = {
    name: storeTestHelper.testTableName,
    columnDefinitions: {
      id: MobileServiceSqliteStore.ColumnType.Integer,
...
}), $test('Verify MobileServiceSqliteStore initialization works as expected with the new operator').check(function() {
  var store = new MobileServiceSqliteStore('somedbname');
  $assert.isNotNull(store.upsert);
}), $test('Verify MobileServiceSqliteStore initialization works as expected without the new operator').check(function() {
  var store = MobileServiceSqliteStore('somedbname');
  $assert.isNotNull(store, 'somedbname');
  $assert.isNotNull(store.upsert);
}), $test('MobileServiceSqliteStore constructor without db name').check(function() {
  var localStore = MobileServiceSqliteStore('somedbname');
  $assert.isNotNull(store.upsert);
}), $test('MobileServiceSqliteStore constructor with a null db name').check(function() {
  var localStore = MobileServiceSqliteStore('somedbname');
  $assert.isNotNull(store.upsert);
}));
