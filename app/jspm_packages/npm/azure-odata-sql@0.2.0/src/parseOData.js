...
var ODataParser = types.defineClass(ctor, {
  ...
  _parseMultiplicative: function() {
    ...
    while (this.token.id == 'Multiply' || this.token.id == 'Divide' || this.token.id == 'Modulo') {
      var opId = this.token.id;
      this._nextToken();
  ...
  _parseUnary: function() {
    if (this.token.id == 'Minus' || this.token.id == 'Not') {
      ...
      this._nextToken();
      if (opId == 'Minus' && (this.token.id == 'IntegerLiteral' || this.token.id == 'RealLiteral')) {
  ...
  _validateFunctionParameters: function(functionName, args, expectedArgCount) {
    if (args.length !== expectedArgCount) {
      var error = util.format("Function '%s' requires %d %s", functionName, expectedArgCount, (expectedArgCount > 1) ? "parameters." : "parameter.");
