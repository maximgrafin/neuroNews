...
$testGroup('push', $test('register makes PUT HTTP request with URL containing installation ID').checkAsync(function() {
...
}), $test('unregister makes DELETE HTTP request with URL containing installation ID').checkAsync(function() {
  return client(function(req) {
    $assert.areEqual(req.type, 'DELETE');
    $assert.areNotEqual(req.url.indexOf(encodeURIComponent(MobileServiceClient._applicationInstallationId)), -1);
  }).push.unregister('channelUri');
}), $test('register sets data properties').checkAsync(function() {
...
}), $test('templates bodies are stringified if not already a string').checkAsync(function() {
  var body = {text: 'test'};
  return client(function(req) {
    var data = JSON.parse(req.data);
...
function client(tests) {
  return new MobileServiceClient("http://www.test.com").withFilter(function(req, next, callback) {
    tests(req);
