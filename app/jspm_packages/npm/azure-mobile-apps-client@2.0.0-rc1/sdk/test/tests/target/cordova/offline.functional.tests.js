...
var testTableName = storeTestHelper.testTableName,
    ...
    id,
    store;
$testGroup('offline functional tests').functional().beforeEachAsync(function() {
...
}).tests($test('Basic push - insert / update / delete').description('performs insert, update and delete on the client and pushes each of them individually').checkAsync(function() {
  var actions = ['clientinsert', 'push', 'serverlookup', function(result) {
    $assert.isNotNull(clientValue);
    $assert.areEqual(result.id, clientValue.id);
    $assert.areEqual(result.text, clientValue.text);
  }, 'clientupdate', 'push', 'serverlookup', function(result) {
    $assert.isNotNull(clientValue);
    $assert.areEqual(result.id, clientValue.id);
    $assert.areEqual(result.text, clientValue.text);
...
}), $test('vanilla pull - insert / update / delete').description('performs insert, update and delete on the server and pulls each of them individually').checkAsync(function() {
  var actions = ['serverinsert', 'vanillapull', 'clientlookup', function(result) {
    $assert.areEqual(result.id, serverValue.id);
    $assert.areEqual(result.text, serverValue.text);
  }, 'serverupdate', 'vanillapull', 'clientlookup', function(result) {
    $assert.areEqual(result.id, serverValue.id);
    $assert.areEqual(result.text, serverValue.text);
  ...
  return performActions(actions);
}), $test('Vanilla pull - default page size').checkAsync(function() {
  return performPull(60, 'vanillapull');
}), $test('Vanilla pull - tiny page size').checkAsync(function() {
  pullPageSize = 1;
  return performPull(5, 'vanillapull');
}), $test('Vanilla pull - zero records').checkAsync(function() {
  pullPageSize = 4;
  return performPull(0, 'vanillapull');
}), $test('Vanilla pull - single page').checkAsync(function() {
  pullPageSize = 4;
  return performPull(2, 'vanillapull');
}), $test('Vanilla pull - record count exactly matches page size').checkAsync(function() {
  pullPageSize = 4;
  return performPull(4, 'vanillapull');
}), $test('Vanilla pull - multiple pages').checkAsync(function() {
  pullPageSize = 2;
  return performPull(4, 'vanillapull');
}), $test('Incremental pull - default page size').checkAsync(function() {
  return performPull(60, 'incrementalpull');
}), $test('Incremental pull - tiny page size').checkAsync(function() {
  pullPageSize = 1;
  return performPull(5, 'incrementalpull');
}), $test('Incremental pull - zero records').checkAsync(function() {
  return performPull(0, 'incrementalpull');
}), $test('Incremental pull - single page').checkAsync(function() {
  pullPageSize = 4;
  return performPull(2, 'incrementalpull');
}), $test('Incremental pull - record count exactly matches page size').checkAsync(function() {
  pullPageSize = 4;
  return performPull(4, 'incrementalpull');
}), $test('Incremental pull - multiple pages').checkAsync(function() {
  pullPageSize = 2;
  return performPull(4, 'incrementalpull');
}), $test('Push with response 409 - conflict not handled').description('verifies that push behaves as expected if error is 409 and conflict is not handled').checkAsync(function() {
  var actions = ['serverinsert', 'clientinsert', 'push', function(conflicts) {
    $assert.areEqual(conflicts.length, 1);
    $assert.areEqual(conflicts[0].getError().request.status, 409);
  }];
  return performActions(actions);
}), $test('Push with response 409 - conflict handled').description('verifies that push behaves as expected if error is 409 and conflict is handled').checkAsync(function() {
  ...
  }, 'serverlookup', function(result) {
    $assert.areEqual(result.id, clientValue.id);
    $assert.areEqual(result.text, clientValue.text);
  }];
  return performActions(actions);
}), $test('Push with response 412 - conflict not handled').description('verifies that push behaves as expected if error is 412 and conflict is not handled').checkAsync(function() {
  var actions = ['serverinsert', 'vanillapull', 'serverupdate', 'clientupdate', 'push', function(conflicts) {
    $assert.areEqual(conflicts.length, 1);
    $assert.areEqual(conflicts[0].getError().request.status, 412);
  }];
  return performActions(actions);
}), $test('Push with response 412 - conflict handled').description('verifies that push behaves as expected if error is 412 and conflict is handled').checkAsync(function() {
  syncContext.pushHandler = {};
  syncContext.pushHandler.onConflict = function(pushError) {
  ...
  return performActions(actions);
}), $test('Push - connection error unhandled').checkAsync(function() {
  ...
  return performActions(actions);
}), $test('Push - connection error handled').checkAsync(function() {
  var fail = true;
  filter = function(req, next, callback) {
    if (fail) {
      callback(null, {
  ...
  var actions = ['clientinsert', 'push', 'serverlookup', function(serverValue) {
    $assert.isNotNull(serverValue);
    $assert.areEqual(serverValue.text, clientValue.text);
  }];
  return performActions(actions);
}), $test('Pull - pending changes on client').description('Verifies that pull leaves records that are edited on the client untouched').checkAsync(function() {
  var actions = ['serverinsert', 'vanillapull', 'serverupdate', 'clientupdate', 'vanillapull', 'clientlookup', function(result) {
    $assert.areNotEqual(result.text, serverValue.text);
  }];
  return performActions(actions);
}), $test('pushError.update() test').description('Verifies correctness of error handling function pushError.update()').checkAsync(function() {
  syncContext.pushHandler = {};
  syncContext.pushHandler.onConflict = function(pushError) {
    $assert.areEqual(pushError.getError().request.status, 412);
    var newValue = pushError.getClientRecord();
  ...
  return performActions(actions);
}), $test('pushError.cancelAndUpdate() test').description('Verifies correctness of error handling function pushError.cancelAndUpdate()').checkAsync(function() {
  syncContext.pushHandler = {};
  syncContext.pushHandler.onConflict = function(pushError) {
    $assert.areEqual(pushError.getError().request.status, 412);
    var newValue = pushError.getClientRecord();
  ...
  return performActions(actions);
}), $test('pushError.cancelAndDiscard() test').description('Verifies correctness of error handling function pushError.cancelAndDiscard()').checkAsync(function() {
  syncContext.pushHandler = {};
  syncContext.pushHandler.onConflict = function(pushError) {
    $assert.areEqual(pushError.getError().request.status, 412);
    var newValue = pushError.getClientRecord();
  ...
  return performActions(actions);
}), $test('Multiple records pushed, one conflict - conflict handled').description('Verifies that a conflict, if handled, does not prevent other records from being pushed').checkAsync(function() {
  syncContext.pushHandler = {};
  syncContext.pushHandler.onConflict = function(pushError) {
    $assert.areEqual(pushError.getError().request.status, 412);
    var newValue = pushError.getClientRecord();
  ...
  var serverId1,
      ...
      clientValue2,
      clientValue3;
  var actions = ['serverinsert', 'vanillapull', 'clientupdate', function() {
    serverId1 = serverValue.id;
    clientValue1 = clientValue;
    currentId = generateGuid();
  }, 'serverinsert', 'vanillapull', 'serverupdate', 'clientupdate', function() {
    serverId2 = serverValue.id;
    clientValue2 = clientValue;
    currentId = generateGuid();
  }, 'serverinsert', 'vanillapull', 'clientupdate', function() {
  ...
  return performActions(actions);
}), $test('Multiple records pushed, one conflict - conflict not handled').description('Verifies that a conflict, if unhandled, does not prevent other records from being pushed').checkAsync(function() {
  var serverId1,
      serverId2,
      serverId3,
      clientValue1,
  ...
  return performActions(actions);
}), $test('Push - A handled conflict should be considered handled if isHandled is set to false').checkAsync(function() {
  syncContext.pushHandler = {};
  syncContext.pushHandler.onConflict = function(pushError) {
    $assert.areEqual(pushError.getError().request.status, 412);
    var newValue = pushError.getClientRecord();
  ...
  return performActions(actions);
}), $test('Push - A handled conflict should be considered unhandled if onConflict fails').checkAsync(function() {
  syncContext.pushHandler = {};
  syncContext.pushHandler.onConflict = function(pushError) {
    $assert.areEqual(pushError.getError().request.status, 412);
    var newValue = pushError.getClientRecord();
  ...
  return performActions(actions);
}), $test('Conflict detection - insert').checkAsync(function() {
  return verifyConflict(['serverinsert', 'clientinsert', 'push'], 409);
}), $test('Conflict detection - update').checkAsync(function() {
  return verifyConflict(['serverinsert', 'vanillapull', 'serverupdate', 'clientupdate', 'push'], 412);
}), $test('Conflict detection - delete').checkAsync(function() {
...
function performAction(chain, action) {
  ...
  return chain.then(function(result) {
    ...
    switch (action) {
      ...
      case 'vanillapull':
        return syncContext.pull(query, null, {pageSize: pullPageSize});
...
function performPull(recordCount, pullType) {
  ...
  }, pullType, 'clientread', function(result) {
