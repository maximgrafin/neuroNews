...
var operationTableName = tableConstants.operationTableName,
    ...
    filter,
    client;
$testGroup('push tests').beforeEachAsync(function() {
...
}).tests($test('Local insert - verify X-ZUMO-FEATURES').checkAsync(function() {
...
}), $test('Local update - verify X-ZUMO-FEATURES').checkAsync(function() {
  var table = client.getSyncTable(storeTestHelper.testTableName);
...
}), $test('Local delete - verify X-ZUMO-FEATURES').checkAsync(function() {
  var table = client.getSyncTable(storeTestHelper.testTableName);
  return store.upsert(storeTestHelper.testTableName, {
    id: '1',
    price: 2
...
}), $test('Verify push uses correct version - insert').checkAsync(function() {
  return pushAndValidateIfMatch('insert');
}), $test('Verify push uses correct version - update').checkAsync(function() {
  return pushAndValidateIfMatch('update');
}), $test('Verify push uses correct version - delete').checkAsync(function() {
  return pushAndValidateIfMatch('delete');
}), $test('Retry count: special case - successful in the first attempt').checkAsync(function() {
  var table = client.getSyncTable(storeTestHelper.testTableName);
  return pushAndValidateRetryCount([200], 1, [200], 1);
}), $test('Retry count: pushing first record successful after one retry').checkAsync(function() {
  var table = client.getSyncTable(storeTestHelper.testTableName);
  return pushAndValidateRetryCount([412, 200], 2, [200], 1);
}), $test('Retry count: retry limit hit before first record is successfully pushed').checkAsync(function() {
  var table = client.getSyncTable(storeTestHelper.testTableName);
  return pushAndValidateRetryCount([412, 412, 412, 412, 412, 200], 6, [200], 1);
}), $test('Retry count: push limit almost hit while pushing first record as well as second record').description('verifies that retry count is reset after first record push is complete').checkAsync(function() {
  var table = client.getSyncTable(storeTestHelper.testTableName);
  return pushAndValidateRetryCount([412, 412, 412, 412, 200], 5, [412, 412, 412, 412, 200], 5);
}), $test('Retry count: response status code 500, successful after few retries').checkAsync(function() {
  var table = client.getSyncTable(storeTestHelper.testTableName);
...
function pushAndValidateFeatures() {
  ...
  return client.getSyncContext().push().then(function() {
    ...
    $assert.isTrue(offlineFeatureAdded);
  }).then(function() {
    filterInvoked = false;
    ...
    $assert.isTrue(filterInvoked);
    $assert.isFalse(offlineFeatureAdded);
  });
}
