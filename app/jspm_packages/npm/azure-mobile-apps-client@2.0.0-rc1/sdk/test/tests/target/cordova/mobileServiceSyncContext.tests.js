...
$testGroup('MobileServiceSyncContext tests').beforeEachAsync(function() {
  return storeTestHelper.createEmptyStore().then(function(emptyStore) {
    store = emptyStore;
  });
}).tests($test('Lookup test').description('test verifies that lookup calls store.lookup and returns its result').checkAsync(function() {
...
}), $test('Read test').description('test verifies that read calls store.read and returns its result').checkAsync(function() {
  var records = [{id: 1}];
  var read = store.read;
...
}), $test('Insert should record operations in the operation table').description('test verifies that insert executes operations specified by the operation table manager in a single batch').checkAsync(function() {
...
}), $test('Update should record operations in the operation table').description('test verifies that update executes the operations specified by the operation table manager in a single batch').checkAsync(function() {
  store.executeBatch = function(operationBatch) {
    $assert.areEqual(operationBatch, [{
      action: 'upsert',
      tableName: storeTestHelper.testTableName,
...
}), $test('Del should record operations in the operation table').description('test verifies that del executes the operations specified by the operation table manager in a single batch').checkAsync(function() {
  store.executeBatch = function(operationBatch) {
    $assert.areEqual(operationBatch, [{
      action: 'delete',
      tableName: storeTestHelper.testTableName,
...
}), $test('Insert should fail iff object with same id already exists').checkAsync(function() {
  var syncContext;
...
}), $test('Update should work even if record with same id already exists').checkAsync(function() {
  var syncContext;
  return getSyncContext().then(function(context) {
    syncContext = context;
...
}), $test('Inserting object without ID should auto-generate ID').checkAsync(function() {
  store.executeBatch = function(operationBatch) {
...
}), $test('Updating object without ID should fail').checkAsync(function() {
  store.executeBatch = function(operationBatch) {
  ...
  return defineTestTable().then(function() {
    ...
    return performActionWithCustomLogging(undefined, 'update');
  }).then(function(result) {
    $assert.fail('failure expected');
  }, function(error) {});
}), $test('Deleting object without ID should fail').checkAsync(function() {
  store.executeBatch = function(operationBatch) {
    $assert.fail('We should have failed long before this getting called!');
  };
  return defineTestTable().then(function() {
    ...
    return performActionWithCustomLogging(undefined, 'del');
  }).then(function(result) {
    $assert.fail('failure expected');
  }, function(error) {});
}), $test('Verify insert, update, delete do not run concurrently').checkAsync(function() {
  var syncContext,
      actionInProgress,
      operationCount = 0,
      repetitionCount = 1,
  ...
  function notifyWhenDone() {
    if (operationCount === repetitionCount * 3) {
      if (completion) {
        completion();
  ...
  return createSyncContextWithLongExecutionTime(function(action) {
    $assert.isNull(actionInProgress);
...
}), $test('purge').description('Tests that the purge API simply calls pullManager.purge() and returns whatever it returns').checkAsync(function() {
  var syncContext;
...
function performActionWithCustomLogging(id, action) {
  ...
  return getSyncContext().then(function(context) {
    ...
    if (action === 'lookup') {
      ...
      return syncContext.insert(storeTestHelper.testTableName, {
        id: id,
        name: testName
      });
    } else if (action === 'update') {
      return syncContext.update(storeTestHelper.testTableName, {
        id: id,
        name: testName
      });
    } else if (action === 'del') {
