...
var SqlFormatter = types.deriveClass(ExpressionVisitor, ctor, {
  ...
  _formatPagedQuery: function(query) {
    ...
    var tableName = helpers.formatTableName(this.schemaName, query.table);
    formattedSql = util.format("SELECT %s " + "FROM %s " + "WHERE %s " + "ORDER BY %s " + "OFFSET %d ROWS FETCH NEXT %d ROWS ONLY", selection, tableName, filter, ordering, query.skip, query.take);
    return formattedSql;
  },
