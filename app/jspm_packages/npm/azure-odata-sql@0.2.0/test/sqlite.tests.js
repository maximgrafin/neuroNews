...
(function(Buffer) {
  ...
  describe('azure-odata-sql.sqlite', function() {
    ...
    it("adds totalCount query when specified", function() {
      var query = {
        ...
        includeTotalCount: true
          ...
          expectedSql = ["SELECT * FROM [intIdMovies] LIMIT 11 OFFSET 10", "SELECT COUNT(*) AS [count] FROM [intIdMovies]"];
      verifySqlFormatting(query, expectedSql);
    });
    ...
    it("inline count with paging and filter", function() {
      ...
      var expectedSql = ["SELECT [title], [type], [price] FROM [books] WHERE (([type] = @p1) AND ([price] < @p2)) LIMIT 5 OFFSET 4", "SELECT COUNT(*) AS [count] FROM [books] WHERE (([type] = @p3) AND ([price] < @p4))"];
    ...
    it("string concat", function() {
      ...
      var statements = verifySqlFormatting(query, "SELECT * FROM [customers] WHERE ((([city] || @p1) || [country]) = @p2)");
      equal(statements.length, 1);
