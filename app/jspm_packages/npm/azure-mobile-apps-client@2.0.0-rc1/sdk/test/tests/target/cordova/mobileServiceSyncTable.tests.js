...
var store,
    table,
    client;
$testGroup('MobileServiceSyncTable tests').beforeEachAsync(function() {
...
}).tests($test('table.where operator').checkAsync(function() {
  var record1 = {
    id: '1',
    text: 'abc'
  },
  ...
  return store.upsert(storeTestHelper.testTableName, [record1, record2, record3, record4]).then(function() {
    return table.where({text: 'abc'}).orderByDescending('id').read();
...
}), $test('insert').description('Verifies that insert acts as a passthrough function').check(function() {
...
}), $test('lookup').description('Verifies that lookup acts as a passthrough function').check(function() {
  var testId = 4321,
      testResult = 'operation result',
...
}), $test('read').description('Verifies that read acts as a passthrough function').check(function() {
...
}), $test('update').description('Verifies that update acts as a passthrough function').check(function() {
  var testRecord = {somekey: 'some value'},
      testResult = 'operation result';
...
}), $test('del').description('Verifies that del acts as a passthrough function').check(function() {
  var testRecord = {somekey: 'some value'},
      testResult = 'operation result';
...
}), $test('pull - query specified').description('Tests that the pull API simply calls MobileServiceSyncContext.pull() and returns whatever it returns').check(function() {
  var query = 'query',
      queryId = 'queryId',
      settings = 'settings',
      result = 'result';
...
}), $test('pull - query not specified').description('Tests that the pull API simply calls MobileServiceSyncContext.pull() and returns whatever it returns').check(function() {
  var queryId = 'queryId',
      settings = 'settings',
      result = 'result';
  client.getSyncContext().pull = function(queryParam, queryIdParam, settingsParam) {
...
}), $test('purge - query specified').description('Tests that the purge API simply calls MobileServiceSyncContext.purge() and returns whatever it returns').check(function() {
