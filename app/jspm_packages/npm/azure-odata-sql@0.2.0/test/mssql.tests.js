...
(function(Buffer) {
  ...
  describe('azure-odata-sql.mssql', function() {
    ...
    it("correctly handles null take", function() {
      var query = {
        ...
        take: null
          ...
          expectedSql = "SELECT * " + "FROM [testapp].[intIdMovies] WHERE ([Duration] = [Duration]) " + "ORDER BY [Title] " + "OFFSET 500 ROWS FETCH NEXT 9007199254740992 ROWS ONLY";
    ...
    it("adds totalCount query when specified", function() {
      var query = {
        ...
        includeTotalCount: true
          ...
          expectedSql = ["SELECT * FROM [testapp].[intIdMovies] WHERE (1 = 1) ORDER BY [id] OFFSET 10 ROWS FETCH NEXT 10 ROWS ONLY", "SELECT COUNT(*) AS [count] FROM [testapp].[intIdMovies]"];
      verifySqlFormatting(query, expectedSql);
    });
    ...
    it("inline count with paging and filter", function() {
      ...
      var expectedSql = ["SELECT [title], [type], [price] FROM [testapp].[books] WHERE (([type] = @p1) AND ([price] < @p2)) ORDER BY [id] OFFSET 4 ROWS FETCH NEXT 4 ROWS ONLY", "SELECT COUNT(*) AS [count] FROM [testapp].[books] WHERE (([type] = @p3) AND ([price] < @p4))"];
    ...
    it("string concat", function() {
      ...
      var statements = verifySqlFormatting(query, "SELECT * FROM [testapp].[customers] WHERE ((CONVERT(NVARCHAR(MAX), (CONVERT(NVARCHAR(MAX), [city]) + @p1)) + [country]) = @p2)");
      equal(statements.length, 1);
    ...
    it("verify function arguments", function() {
      var testCases = [{
        ...
        filters: "ceiling(a, b, c)",
        expectedParamCount: 1
      }, {
        filters: "round(a, b, c)",
        expectedParamCount: 1
      }, {
        filters: "substringof(a)",
        ...
        filters: "substring(a)",
        expectedParamCount: 3,
        expectedError: "Function 'substring' requires 2 or 3 parameters."
      }, {
        filters: "concat()",
        expectedParamCount: 2
      }, {
